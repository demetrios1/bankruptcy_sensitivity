...
BARTpred=function(df, treat=G, Outcome=B,vars, mono=F){
...
for (m in 1:length(gamma_list)){
  set.seed(1022)
set.seed(0)
  N <- 25000 # Number of random samples
...
#colnames(temp_mBG)<-c( "y1", "y0", "d", "tau", "convergence", "fnvalue" ,'B1','B0')
...
p1 = mBG_out[[1]][, 'B1']
p1true=pnorm(gamma_list[]+mu2_list[[1]]) 
...
#write.csv(data.frame(RR_true, RR_est), 'monobart_RR_bivariate_sim.csv')
write.csv(data.frame(RR_true, RR_est), 'monobart_RR_bivariate_sim_2.csv')
...
text(15, 10, paste0('correlation= ',round(cor(RR_est[,1], RR_true[,1]),3)))
