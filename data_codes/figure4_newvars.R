...
q=0.1
#sig=.5
sdlist=seq(from=0.05, to=1, length.out=20)
sdlist




for (m in 1:length(sdlist)){#:length(gamma_list)){
  q=0.1
  var=sdlist[[m]]^2  #the true variance we wanna set
  a=0.3633802
  b=0.7978846
  ...
  #f = function(z){
  #  val = rep(NA, length(z))
  #  val[z < 0] = 2*q*dnorm(z[z<0], sd = s)
  #  val[z>0] = 2*(1-q)*dnorm(z[z>0],0,sd = s*(1-q)/q)
  #  return(val)
  #  
  #}
  ...
  setwd("/home/dpapakos/sensitivity_analysis/constrained_integration_check_newvar/")

  write.csv( mBG_out, file = paste("inducement_normal_mean0_sd", sdlist[[m]], '.csv',sep = ""), row.names = FALSE )
  print(m)
...
RR=intframe$treatpred/intframe$notreatpred
tau_naive=intframe$treatpred-intframe$notreatpred
mean_tau=mean(tau_naive)
mean_RR=mean(intframe$treatpred/intframe$notreatpred)

mean_inducement=sapply(1:20,function(i)mean(file_list[[i]]$B1/file_list[[i]]$B0)/mean_RR)
mean_inducement
list.files()
setwd("/home/dpapakos/sensitivity_analysis/constrained_integration_check_shark_newvar/")
file_list2<-c()
list.files()
library(gtools)
new_files2<-mixedsort(sort(list.files()))

for (i in 1:length(new_files)){
  file=new_files2[[i]]
  file_list2[[i]]<-data.table::fread(file)
}
file_list2
new_files2

setwd("/home/dpapakos/sensitivity_analysis/constrained_integration_check_sharkq0.9_newvar/")
file_list3<-c()
list.files()
library(gtools)
new_files3<-mixedsort(sort(list.files()))

for (i in 1:length(new_files3)){
  file=new_files3[[i]]
  file_list3[[i]]<-data.table::fread(file)
}

RR=intframe$treatpred/intframe$notreatpred
mean_RR=mean(intframe$treatpred/intframe$notreatpred)
mean_tau=mean(intframe$treatpred-intframe$notreatpred)
mean_inducement_shark=sapply(1:20,function(i)mean(file_list2[[i]]$B1/file_list2[[i]]$B0)/mean_RR)
mean_inducement_sharkqpt9=sapply(1:20,function(i)mean(file_list3[[i]]$B1/file_list3[[i]]$B0)/mean_RR)
mean_inducement=sapply(1:20,function(i)mean(file_list[[i]]$B1/file_list[[i]]$B0)/mean_RR)
mean_tau_shark=sapply(1:20,function(i)mean(file_list2[[i]]$B1-file_list2[[i]]$B0)/mean_tau)
mean_tau_sharkqpt9=sapply(1:20,function(i)mean(file_list3[[i]]$B1-file_list3[[i]]$B0)/mean_tau)
mean_tau_norm=sapply(1:20,function(i)mean(file_list[[i]]$B1-file_list[[i]]$B0)/mean_tau)
mean_inducement_shark
sdlist2=rep(sdlist, 3)
mean_inducement_merge=c(mean_inducement, mean_inducement_shark,mean_inducement_sharkqpt9)
mean_inducement_merge
induceoverrisk <- data.frame(sdlist, mean_inducement_merge, f=c(rep('Normal',20), rep('shark q=0.1',20), 
                                              rep('shark q=0.9',20)))%>%
  ggplot(aes(x=sdlist, y=mean_inducement_merge, color=f))+
  stat_summary(geom = 'point')+geom_line()+#geom_smooth(size=0.72, method='loess')+ 
  scale_colour_manual("",labels = c("Normal", "Shark q=0.1", 'Shark q=0.9'), 
     values = c("dodgerblue4", "firebrick4", 'black'))+
  xlab(expression(sigma))+ylab('Inducement/observed Risk Ratio')+theme_minimal()+theme(plot.title = element_text(hjust = 0.5, size=12))+
  ggtitle('Mean Inducement over Mean Observed Risk Ratio for different f')+scale_linetype_manual(values=c('solid', 'dashed', 'solid'), name='')+  
  scale_colour_manual(values=c('firebrick4', 'dodgerblue4', 'black'), name='Distribution')
induceoverrisk

ggsave(filename="induceoverrisk_newvar.pdf",height=4, width=6.5,  plot=induceoverrisk)
induceoverrisk
mean_tau_merge=c(mean_tau_norm, mean_tau_shark,mean_tau_sharkqpt9)
mean_tau_merge
treatoverinduce <- data.frame(sdlist, mean_tau_merge, f=c(rep('Normal',20), rep('shark q=0.1',20), 
                                              rep('shark q=0.9',20)))%>%
  ggplot(aes(x=sdlist, y=mean_tau_merge, color=f))+
  stat_summary(geom = 'point')+geom_line()+#geom_smooth(size=0.72, method='loess')+ 
  scale_colour_manual("",labels = c("Normal", "Shark q=0.1", 'Shark q=0.9'), 
                      values = c("dodgerblue4", "firebrick4", 'black'))+
  xlab(expression(sigma))+ylab('Treatment effect/naive treatment effect')+theme_minimal()+theme(plot.title = element_text(hjust = 0.5, size=12))+
  ggtitle('Mean treatment effect over mean naive treatment effect for different f')+scale_linetype_manual(values=c('solid', 'dashed', 'solid'), name='')+  
  scale_colour_manual(values=c('firebrick4', 'dodgerblue4', 'black'), name='Distribution')

setwd("/home/dpapakos/sensitivity_analysis/")
#ggsave(filename="treatovernaive.pdf",height=4, width=6.5,  plot=treatoverinduce)
...
scatter_quick <- read.csv('inducement_normal_mean0_sd1.csv')
scatter_quick

plot(scatter_quick$B1, scatter_quick$tau,
     xlab='Pr(B=1|do(G=1))', ylab='RD', cex=0.5, pch=1, main='f(u)~N(0,1)')
print( proc.time() - ptm )
