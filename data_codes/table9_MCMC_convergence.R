...
# #cast the mono bart stuff to coda object
# mono_pr0 <- mcmc(bart_mono$pr0[,1])
# acf(mono_pr0)[5]
# geweke.plot(mono_pr0)
# geweke.diag(mono_pr0)
...
# covtreat=covariates%>%filter(G==1)
# covcontrol=covariates%>%filter(G==0)
# covtreat$G=as.factor(covtreat$G)
# vars=c('x1','x2','x3','x4','x5')
# xtest=covariates[vars]
# xtraincontrol=covariates[vars]
# xtraintreat=covtreat[vars]
# 
...
# z = 1-c(rep(1, length(ytrain1)), rep(0, length(ytrain0)))
# z
# y = as.numeric(c(ytrain1, ytrain0)==1)
# bart_mono = monotone_bart(y = as.numeric(c(ytrain1, ytrain0)==1),
#                           z = 1-c(rep(1, length(ytrain1)), rep(0, length(ytrain0))),
#                           x = rbind(xtraintreat, xtraincontrol),
#                           xpred = xtest, nskip = 2000, ndpost = 2000,m=100)
# bart_mono_pr0 <- 1-colMeans(bart_mono$pr1)
# bart_mono_pr1 <- 1-colMeans(bart_mono$pr0)
# bart_mono_pr0_ <- colMeans(bart_mono$pr0)
# bart_mono_pr1_ <- colMeans(bart_mono$pr1)
# (bart_mono_pr1/bart_mono_pr0)-(bart_mono_pr1_/bart_mono_pr0_)
# #Pr(B|G=1)
# bart1=bart(covtreat[vars],as.factor(covtreat$B), covariates[vars],
#            ndpost = 2000, nskip = 2000,ntree=100,verbose=T,usequants = TRUE,numcut = 1000)
# 
# #case 2 control
# #Pr(B|G=0)
# bart0=bart(covcontrol[vars],as.factor(covcontrol$B),covariates[vars],
#            ndpost =2000, nskip = 2000,ntree=100,verbose=T,usequants = TRUE,numcut = 1000)
# (bart_mono_pr1_)#-bart_mono_pr0_)
# (bart_mono_pr1)#-bart_mono_pr0)
# plot(colMeans(pnorm(bart1$yhat.test[])), bart_mono_pr1[])
# ####posterior draws x N
# dim(bart_mono$pr0)
# #cast the mono bart stuff to coda object
# mono_pr0 <- mcmc(bart_mono$pr0[,1])
...
# geweke.plot(mono_pr0)
# geweke.diag(mono_pr0)
# mono_pr1 <- mcmc(bart_mono$pr1[,1])
# geweke.plot(mono_pr1)
# geweke.diag(mono_pr1)
# dim(bart1$yhat.test)
# bart_pr0 <- mcmc(pnorm(bart0$yhat.test[,1]))
# geweke.plot(bart_pr0)
# geweke.diag(bart_pr0)
# 
# bart_pr1 <- mcmc(pnorm(bart1$yhat.test[,1]))
# geweke.plot(bart_pr1)
# geweke.diag(bart_pr1)
# traceplot(bart_pr1)
# 
# 
