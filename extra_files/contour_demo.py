...
#rho is 0.420 from Franks code
data = np.random.multivariate_normal((1., 1), [[1, .420], [.420, 1]], 1000000)
...
# Label plot
#how did we get gamma estimate?
#it is the coefficient on gamma!
gamma=0.187
...
ax.vlines(0, 0, -6, lw=1, color='black')
ax.vlines(-1*gamma, 0, 4, lw=1, color='black')
...
ax.text(-1*gamma-0.24, -0.335, '$-\gamma$', fontsize=12)
import matplotlib.transforms as mtransforms
trans = mtransforms.blended_transform_factory(ax.transData, ax.transAxes)
y1positive=y>0
d = scipy.zeros(len(y))

ax.fill_betweenx(y, -1*gamma, 0, where= y>=d, 
                facecolor='grey', alpha=0.5)
#
pl.ylabel('Exposure score: $\Phi^{-1}(Pr(G\mid \mathbf{x}))$')
pl.xlabel('Outcome score: $\Phi^{-1}(Pr(B\mid \mathbf{x}))$')
...
pl.savefig("C:/Users/demetri/Dropbox/dissertation/figures/bivar_norm_2.png")
pl.show()