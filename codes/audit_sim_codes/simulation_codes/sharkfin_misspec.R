...
BARTpred=function(df, treat=G, Outcome=B,vars){
  ...
  bart3 = bart(df[vars], as.factor(df$G),df[vars], ndpost = 2000, nskip = 2000, ntree=100, usequants=T, numcuts=1000, verbose=F)
  
  
  
  #use this method for prediction on binary
  pred1= 1-colMeans(bart_mono$pr0)  #B1,G0
  pred2= 1-colMeans(bart_mono$pr1)#B1, G1
  pred3=colMeans(pnorm(bart3$yhat.test))#P(G)
...
#TABLE 7...entry 1:8 will redo the left hand side where 
#we use the correct specification of f, i.e. f=fcorrect
####cases 9-16, we use incorrect spec, i.e. f\neq fcorrect
...
#  }
  ...
  setwd('/home/dpapakos/sharkfin_bigq_change/')
...
sapply(1:length(qlist), function(i)plot( file_list[[i]]$tau, file_list[[i]]$tau.true, use="complete.obs"))
