...
for (i in 1:length(qlist)){
  ...
  #this is the matrix of dimension n x 3, with each column representing the joint probability
  # and the rows representing the observations
  intframe=BARTpred(covariates, treat=G, Outcome=B,vars, mono=T)

  #convert the frame to integrate over to a matrix
...
df2=data.frame(tau=file_list[[7]]$tau, true=file_list[[7]]$tau.true)%>%
  na.omit()
df2%>%summarize(RMSE=sqrt(mean((tau - true)^2)))

par(mfrow=c(1,1))
plot(file_list[[7]]$tau.true, file_list[[7]]$tau, main='q true, lower variance',
     xlab='true tau', ylab='our guess')
abline(0,1 , col='red')
text(0.07,.5, paste0('cor=',round(cor(file_list[[8]][, 'tau'], file_list[[8]]$tau.true),3)))


setwd('/home/dpapakos/nonlin_sim_holdcsv/')
