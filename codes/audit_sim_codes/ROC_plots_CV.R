...
####NOTE: the 1 refers to B=1|G=0, the 2nd case is B=1|G=1, and the third case is G=1
# load data
getwd()
...
bart1=auc(dd_2$going_concern, bartstuff$gprob)
plot(roc(dd_2$going_concern, bartstuff$gprob))
bart2=auc(dd_2$bankrptobs[dd_2$going_concern==1], 
          bartstuff$bprob_gc1[dd_2$going_concern==1])
plot(roc(dd_2$going_concern, bartstuff$gprob))
bart3=auc(dd_2$bankrptobs[dd_2$going_concern==0], 
          bartstuff$bprob_gc0[dd_2$going_concern==0])
bart3

outcome2
rf=read_csv('outcomemine.csv')
sa        = "lgc_0"
data_sa = data[ !is.na( data[,sa] ), ]     # drop missing values of sample indicators
data_sa = data_sa[ data_sa[,sa] == 1 & data_sa$neg_ni==1, ]
...
mbart_pred_func=function(y_here, x_here, y_here_2, x_here_2){
  ...
  data_sasub$y_here=ifelse(data_sasub$bankrptobs==1&data_sasub$going_concern==0, 3,
                   ifelse(data_sasub$bankrptobs==0&data_sasub$going_concern==0, 2,
                          ifelse(data_sasub$bankrptobs==1&data_sasub$going_concern==1, 1,0)))
  test_index <- purrr::map(1:fold,function(i, y_1, y_0){c(y_1[[i]], y_0[[i]])},
                           y_1=split(sample(which(data_sasub$y_here==3)), 1:fold),
                           y_0=split(sample(which(data_sasub$y_here==2)), 1:fold))
  
  test_index_2 <- purrr::map(1:fold,function(i, y_1, y_0){c(y_1[[i]], y_0[[i]])},
                             y_1=split(sample(which(data_sasub$y_here==1)), 1:fold),
                             y_0=split(sample(which(data_sasub$y_here==0)), 1:fold))
  y_train <- list()
  y_test <- list()
  ...
  for (cv in 1:fold) {
    ...
    train_y_0 = as.factor(y_here)[-test_index[[cv]]]
    test_y_0 = as.factor(y_here)[test_index[[cv]]]
    train_x_1 = t(x_here_2)[var0,-test_index_2[[cv]] ]
    test_x_1 = t(x_here_2)[var0,test_index_2[[cv]] ]
    train_y_1 = as.factor(y_here_2)[-test_index_2[[cv]]]
    test_y_1 = as.factor(y_here_2)[test_index_2[[cv]]]
 ...
 xtest[[cv]]=data_sasub[c(test_index[[cv]],test_index_2[[cv]]), var0]
    ...
    true_test[[cv]]=data_sasub[c(test_index[[cv]],test_index_2[[cv]]), c('bankrptobs', 'going_concern')]
...
x_here=data_sasub
x_here_2=data_sasub
y_here=data_sasub$bankrptobs
y_here_2=data_sasub$bankrptobs
data_sasub$bankrptobs
MBART_CV=mbart_pred_func(y_here=y_here, x_here=x_here, 
                         y_here_2=y_here_2, x_here_2=x_here_2)
...
bart_fit<-function(y_here, x_here){
  ...
  for (cv in 1:fold) {
    ...
    bart_fit=bart(x_train, y_train[[cv]],x_test,ndpost = 2000, nskip = 2000,ntree=100,
...
boost1=auc(dd_2$going_concern, boost$prob_g)
boost2=auc(dd_2$bankrptobs[dd_2$going_concern==1], 
           boost$prob_b1_g1[dd_2$going_concern==1])
boost3=auc(dd_2$bankrptobs[dd_2$going_concern==0], 
           boost$prob_b1_g0[dd_2$going_concern==0])

verification::roc.plot(dd_2$going_concern, bartstuff$gprob)
...
library(viridis)
roc(dd_2$going_concern, bartstuff$gprob)
aa=ggrocs(list(BART=roc(bart3$true_test, bart3$bart_test_prob), 
          ...
          breaks = seq(0,1,0.1), 
          tittle = "ROC perf. predicting P(G)")
...
bb=ggrocs(list(BART=roc(imp_frame$bankrptobs[imp_frame$going_concern==1],imp_frame$BG1[imp_frame$going_concern==1]), 
          ...
          breaks = seq(0,1,0.1), 
          tittle = "ROC perf. predicting P(B|G=1)")
bb
cc=ggrocs(list(BART=roc(dd_2$bankrptobs[dd_2$going_concern==0], 
                        bartstuff$bprob_gc0[dd_2$going_concern==0]), 
               BOOST=roc(dd_2$bankrptobs[dd_2$going_concern==0], 
                         boost$prob_b1_g0[dd_2$going_concern==0]), 
               RF=roc(dd_2$bankrptobs[dd_2$going_concern==0], rando1$votes[, 2])), 
          breaks = seq(0,1,0.1), 
          tittle = "ROC perf. predicting P(B|G=0)")
