...
binded2<-binded2%>%
  mutate(normed1=V2/sum(V2))
# 
rightbump2<-binded2%>% ggplot()+geom_density(aes(x=vu,y=normed1),color='black',stat='identity', lwd=.95)+xlim(-3,3)+labs(colour = "Cylinders")+
  xlab('u')+ylab('f(u)')+
   ggtitle(expression(paste(mu,'=-2,0,2,',  sigma, '=0.48'))) +
  theme(plot.title = element_text(hjust = 0.5))#+annotate('text', x = c(-2,0,2), y =c(.02,.18,.03), label=c("area=0.01 ",'area=0.94','area=0.05'), size=8)#+geom_vline(xintercept=c(c(-2.2,-1.8),c(-0.2,.2),c(1.8,2.2)))
# #setwd("C:/Users/demetri/Documents/ASU/RTG/fixed_plots")
