...
#write.csv(intframe, "/home/dpapakos/moderating_variables/mbart_audit.csv")
intframe=data.table::fread("/home/dpapakos/moderating_variables/mbart_audit.csv")
...
for (m in 1:length(sdlist)){#:length(gamma_list)){
  ...
  setwd("/home/dpapakos/sensitivity_analysis/constrained_integration_check_sharkq0.9/")

  write.csv( mBG_out, file = paste("inducement_shark_q0.9_sd", sdlist[[m]], '.csv',sep = ""), row.names = FALSE )
  print(m)

}

setwd("/home/dpapakos/sensitivity_analysis/constrained_integration_check/")
file_list<-c()
list.files()
library(gtools)
...
setwd("/home/dpapakos/sensitivity_analysis/constrained_integration_check_shark/")
file_list2<-c()
list.files()
library(gtools)
...
ggsave(filename="induceoverrisk.pdf",height=4, width=6.5,  plot=induceoverrisk)
induceoverrisk
