...
for (i in 1:length(qlist)){
  ...
  setwd('/home/dpapakos/nonlin_sim_incorrect_codes/nonlim_sim_constrained_shark')
  write.csv(saveframe,paste0('correct_f_', i+16, '_sharkfin_q=',q,'correct_var=', var,
      'qfalse=',qfalse,'varfalse=', varfalse,             '.csv'))
}
...
new_files
M=length(new_files)+1

colnames(file_list[[1]])
tau.est=sapply(1:(M-1),function(i)file_list[[i]]$B1-file_list[[i]]$B0)
tau.est_mean=sapply(1:(M-1),function(i)mean(file_list[[i]]$B1-file_list[[i]]$B0))
tau.true=sapply(1:(M-1),function(i)file_list[[i]]$B1.true-file_list[[i]]$B0.true)
tau.true_mean=sapply(1:(M-1),function(i)mean(file_list[[i]]$B1.true-file_list[[i]]$B0.true))
RR_est=sapply(1:(M-1),function(i)file_list[[i]]$B1/file_list[[i]]$B0)
RR_true=sapply(1:(M-1), function(i)file_list[[i]]$B1.true/file_list[[i]]$B0.true)
...
tau.est_mean
library(xtable)
View(data.frame(tau.true_mean,tau.est_mean, tau_RMSE,tau_cor, 
...
new_df=lapply(1:length(qlist),function(i)data.frame(file_list[[i]], B=B_list[[i]], G=G_list[[i]]))

data.frame(sapply(1:16,function(i)new_df[[i]]%>%dplyr::filter(G==1)%>%summarize(ATT_guess=mean(tau),
                                                                                    ATT_true=mean(tau.true))))


data.frame(sapply(1:16,function(i) new_df[[i]]%>%dplyr::filter(G==0)%>%summarize(ATC_guess=mean(tau),
                                                                                    ATC_true=mean(tau.true))))
